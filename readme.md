...
## 更新日誌
### 2024-01-30 (v1.0.0-stable-20250130_lucky_7pm)
- [x] 修復語言設定流程問題
  - 修正語言設定順序：A → B → C
  - 優化資料庫結構，修正 UNIQUE 約束
  - 改進使用者提示訊息
- [x] 資料庫優化
  - 修正 language_settings 表的唯一約束
  - 優化資料庫操作效率
  - 加強錯誤處理機制
- [x] 穩定性改進
  - 完善錯誤處理流程
...
### ⚠️ 重要注意事項（請勿刪除）
1. 資料庫結構
   - `language_settings` 表使用 `UNIQUE(context_id, context_type)` 約束
   - 請勿修改此約束，這是確保多群組/私聊正常運作的關鍵

2. 語言設定流程
   - 設定順序必須保持：A → B → C
   - 請勿更改此順序，這是確保使用者體驗的重要機制

3. 穩定版本標記
   - 最新穩定版本：`20250130_lucky_7pm`
   - 此版本已經過完整測試，請勿隨意更改核心功能

4. 關鍵功能提醒
   - 資料庫操作必須包含 `context_id` 和 `context_type`
   - 語言設定更新必須使用正確的 SQL 語句
   - 錯誤處理必須保留完整的日誌記錄
